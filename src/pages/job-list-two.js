...
const JobListTwo = () => {
    ...
    useEffect(() => {
        const filterJobs = () => {
            const filtered = jobData.filter((job) => {
                ...
                const matchesLocation = selectedLocation ? job.state === selectedLocation : true;
        ...
        filterJobs();
    }, [jobData, searchKeyword, selectedCategory, selectedLocation, selectedJobTypes]);

    const handleSearchChange = (e) => setSearchKeyword(e.target.value);
    const handleCategoryChange = (e) => setSelectedCategory(Number(e.target.value));
    ...
    const handleUpdateJob = async (updatedJob) => {
        try {
            const response = await api.put(`/jobs/update/${updatedJob.id}`, updatedJob);
            setFilteredJobs(filteredJobs.map((job) => (job.id === updatedJob.id ? response.data : job)));
            setEditingJob(null);
        } catch (error) {
                                    ...
                                    {jobTypes.map((jobType) => (
                                        <div className="d-flex justify-content-between mt-2" key={jobType.jobTypeId}>
                                            <div className="form-check">
                                                ...
                                                />
                                                <label className="form-check-label" htmlFor={jobType.jobTypeId}>
                                                    {jobType.jobTypeName}
                                                </label>
                                            </div>
                                        </div>
                                            ...
                                            <div className="job-post job-post-list rounded shadow p-4 d-md-flex align-items-center justify-content-between position-relative">
                                                ...
                                                <div className="mt-3 mt-md-0">
                                                    ...
                                                    {enterpriseRole && (
                                                        <>
                                                            <Link to="/candidate-applied-list" className="btn btn-sm btn-primary w-full ms-md-1">See CV</Link>
                                                            <button onClick={() => handleEditJob(item)} className="btn btn-sm btn-warning ms-md-1">Edit</button>
                                                            <button onClick={() => deleteJob(item.id)} className="btn btn-sm btn-danger ms-md-1">Delete</button>
                                                        </>
                                                    )}
                                                </div>
                                            </div>
